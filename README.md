...
* **Statistics API (Python/FastAPI):** The real-time stream analytics engine. Implements the **Write-Behind Caching** pattern (aggregating state deltas in Redis before batch-flushing to PostgreSQL) to protect the persistent storage from high I/O write loads during heavy streaming.
