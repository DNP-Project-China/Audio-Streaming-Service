...
def get_track_status(track_id: UUID) -> str:
    ...
    Returns the status as a string (e.g., 'pending', 'processing', 'ready', 'failed') or None if the track is not found.
    """
    status = None
    try:
...
def update_track_status(track_id: UUID, status: str) -> bool:
    """
    Updates the status of a track in the database. If setting status to 'processing', it will only update if the current status is 'pending' or 'failed'.
    This ensures that only one worker can claim a track for processing, preventing race conditions in a distributed environment.
    Returns True if the status was updated successfully, or False if the track was already claimed by another worker or if an error occurred.
    """
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # If rows_updated is 0 when trying to set to 'processing', it means another worker has already claimed this track.
    ...
    # Catch any exceptions that occur during database operations and log them. Return False to indicate the update was not successful.
    except Exception as e:
        print(f"[DB ERROR] {track_id}: {e}")
        return False
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Check how many rows were updated. If 0, it means the track was not in 'processing' status, which could indicate a logic error in the workflow.
                rows_updated = cur.rowcount
            conn.commit()
